# Main script to orchestrate DuppyAI functionalities
...
Create-EventLogSource -source "DuppyAI"
...
if ($modelOrCommand -eq "-GetErrors") {
    ...
    if (-not [string]::IsNullOrWhiteSpace($errorText)) {
        ...
        Write-Host "DuppyAI Analysis: $response"
        Log-Response -response $response
        Copy-ToClipboard -text $response
        Log-Event -message "DuppyAI analysis of error events for '$application' has been logged and copied to clipboard."
...
if ($modelOrCommand -eq "-CheckUpdates") {
    ...
    Write-Host "DuppyAI Analysis: $updateSummary"
    Log-Response -response $updateSummary
    Copy-ToClipboard -text $updateSummary
    Log-Event -message "DuppyAI analysis of updates has been logged and copied to clipboard."
...
Write-Host "DuppyAI ($model): $response"
Log-Response -response $response
Copy-ToClipboard -text $response
Log-Event -message "DuppyAI response logged and copied to clipboard."
